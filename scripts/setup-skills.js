...
// 全局安装 playwright-cli 并预装 chromium，使配套 skill 开箱即用（失败不阻塞）
function setupPlaywrightCli() {
	const packageRoot = path.join(__dirname, "..");
	...
	if (!fs.existsSync(localPkg)) {
		console.log(
			"[setup-skills] @playwright/cli not installed locally, skipping global setup",
		);
		return;
	}
	...
	try {
		// 与本地依赖锁同版本，避免全局版本漂移
		const version = JSON.parse(fs.readFileSync(localPkg, "utf-8")).version;
		execSync(`npm install -g @playwright/cli@${version} --no-audit --no-fund`, {
			stdio: "pipe",
		});
		console.log(`[setup-skills] Installed playwright-cli@${version} globally`);
	} catch (err) {
		const message = err instanceof Error ? err.message : String(err);
		console.error(
			"[setup-skills] Failed to install playwright-cli globally:",
			message,
		);
		return;
	}
	...
	try {
		execSync("playwright-cli install-browser chromium", { stdio: "pipe" });
		console.log("[setup-skills] Playwright chromium browser ready");
	} catch (err) {
		const message = err instanceof Error ? err.message : String(err);
		console.error(
			"[setup-skills] Browser install skipped (run `playwright-cli install-browser chromium` manually):",
			message,
		);
	}
}
...
function syncAgentsMd() {
	...
	if (!fs.existsSync(source)) {
		console.log(
			"[setup-skills] No global/AGENTS.md found, skipping AGENTS.md sync",
		);
		return;
	}
